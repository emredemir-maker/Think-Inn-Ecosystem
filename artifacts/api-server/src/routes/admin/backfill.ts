 ...
 * POST /api/admin/backfill-community  (super_admin only)
  ...
  async (_req, res) => {
    try {
      const ideas = await db
        .select({ id: ideasTable.id, title: ideasTable.title, description: ideasTable.description })
        .from(ideasTable);
      const research = await db
        .select({ id: researchTable.id, title: researchTable.title, summary: researchTable.summary })
        .from(researchTable);
